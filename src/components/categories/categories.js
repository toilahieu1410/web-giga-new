...
import { getListProduct, getCategoryBySlug, getCategoryByDetail, getSubCategory } from "../../redux/product/action"
...
const Categories = () => {
  ...
  useEffect(() => {
    ...
    const scrollTopNextPage = window.setTimeout(() => {
      let typeCategory = ''
      if (listCategorySlug) {
        if (listCategorySlug.type == 0) { typeCategory = "categorySlug" }
        if (listCategorySlug.type == 1) { typeCategory = "subCategorySlug" }
        if (listCategorySlug.type == 2) { typeCategory = "chidrenCategorySlug" }
      }
      const params = {
        type: typeCategory,
        slug: categorySlug,
      ...
      if (typeCategory !== "") {
        dispatch(getListProduct(params, newFilter)) 
         dispatch(getListProduct(params2, newFilter))
     } 
    }, 300)
  ...
  }, [listCategorySlug, categorySlug, page, filter, sort])
